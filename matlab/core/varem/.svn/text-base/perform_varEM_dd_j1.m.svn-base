function [params, hvars, data, pointer] = perform_varEM_dd_j1(map, cluster, rho, file_name, params, hvars, data, pointer)
...
if ( nargin < 5)
    ...
    for counter = 1:data.nos.S
        ...
        data.degrees{counter} = map.degrees{counter};                      % <---------------- d_i d_j
        data.degrees_mean(counter) = mean(data.degrees{counter}(:));    
    end
    ...
    params = updateClusters_new(data,params,hvars);
...
while (counter<31)
% while ( ((abs(ImprovementRatio) >.00000001) | (counter < 20)) & (counter < 30) )
% _dddd -> 0.0001
% _dddd_long -> 0.00001
...
%     if mod(counter,10)==0
%        save(['IntermedResult_VarEM_debugdanial_t' num2str(diffusion_time) '_K' num2str(params.K) '_D' num2str(data.nos.D) '_rho', num2str(rho), '_It' num2str(counter)], ...
