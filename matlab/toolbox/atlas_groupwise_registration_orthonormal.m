function [atlas, mymaps,p] = atlas_groupwise_registration_orthonormal(mymaps, p, verbose)
%
    ...
    %% 4.b) recalculate the transformations
        ...
        for j=1:n_subjects
            disp([ 'average template - ' num2str(j)])
                ...
                if (strcmp(p.matching_type,'spatial')) % use proximity in space as a prior
                    [pair_distances, pair_indices_target, pair_indices_source] = atlas_sparse_cross_distance(positions_template', positions{j}', p.matching_thresh, verbose);
                    epsilon = median(pair_distances);
                    pair_weights = exp(-pair_distances/epsilon);
                else % use correlation of signals as a prior
                ...
                [rotation, translation] = atlas_weighted_align_shapes_nd(funcmaps_template(pair_indices_target,2:end), funcmaps{j}(pair_indices_source, 2:end), pair_weights, p.linear_transform_type);
