function [params, hvars, data, pointer] = perform_varEM_dd_fullcov_nirevision(map, cluster, rho, file_name, params, hvars, data, pointer)
...
mymap = map;
mymap.Phi = mymap.Gamma;
mymap = rmfield(mymap,'Gamma');
...
if ( nargin < 4)
    ...
    data.nos.S = length(mymap.SubjectIndices); % number of subjects
    ...
    for counter = 1:data.nos.S
        ...
        data.degrees{counter} = mymap.degrees{counter};
